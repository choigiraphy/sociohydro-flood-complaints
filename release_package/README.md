...
- Open-access archive (Zenodo): https://doi.org/10.5281/zenodo.19176504
- Development mirror (GitHub): https://github.com/choigiraphy/sociohydro-flood-complaints
- Access conditions: no registration required for the archived reproducibility package
- License: MIT
...
```bash
python3 code/render_all_wrr_figures.py
```
...
- `figure6_source.xlsx`: region-event complaint and precipitation intensity keyed to the national province boundary shapefile
- `figure7_source.xlsx`: region-event focal-emotion shifts keyed to the national province boundary shapefile

These files are the highest-shareable analytical inputs used to support figure reproducibility without redistributing raw complaint text. Geometry is loaded from the province boundary shapefile documented in each file's metadata sheet rather than stored inside the workbook.
