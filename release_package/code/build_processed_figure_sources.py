...
DATA_REPO = Path("/Volumes/Keywest_JetDrive/데이터저장소")
...
FOCAL_EMOTIONS = ["Suspicion/Mistrust", "Anger/Rage", "Sadness"]
...
def build_figure6_source() -> FigureSource:
    ...
    _write_excel(
        out,
        {
            "metadata": _meta_sheet(
                {
                    "figure": "Figure 6",
                    "description": "Regional complaint intensity and precipitation intensity inputs keyed to the national province boundary shapefile.",
    ...
    return FigureSource("Figure 6", out, [str(complaint_src), str(precip_src), str(pop_src), str(shape_src)], ["metadata", "region_event_intensity"], {"region_event_intensity": len(result)}, "One file includes the regional attributes required for the spatial intensity figure; geometry is loaded from the province boundary shapefile listed in metadata.")


def build_figure7_source() -> FigureSource:
    score_src = Path("/Volumes/Keywest_JetDrive/학교/2025-2/논문/figures/figure7_region_score_deltas.xlsx")
    shape_src = DATA_REPO / "final_final/LX법정구역경계_시도_전국/SIDO.shp"
    gdf = _load_shape_with_region()

    rows = []
    for event, sheet_name in [("Flood 2022", "DeltaScore_2022"), ("Flood 2023", "DeltaScore_2023")]:
        wide = pd.read_excel(score_src, sheet_name=sheet_name)
        wide = wide.rename(columns={"Region": "region"})
        wide = wide.loc[wide["region"].notna() & (wide["region"] != "NONE")].copy()
        for emotion in FOCAL_EMOTIONS:
            merged = gdf[["region_eng", "ctpr_nm", "geometry"]].copy()
            merged["event"] = event
            merged["emotion"] = emotion
            value_map = wide.set_index("region")[emotion].to_dict() if emotion in wide.columns else {}
            merged["delta_prop"] = merged["ctpr_nm"].map(value_map)
            merged["meets_min_count"] = merged["delta_prop"].notna()
            rows.append(merged.drop(columns="geometry"))
    result = pd.concat(rows, ignore_index=True)
    ...
    _write_excel(
        out,
        {
            "metadata": _meta_sheet(
                {
                    "figure": "Figure 7",
                    "description": "Regional focal-emotion flood-minus-baseline shifts keyed to the national province boundary shapefile.",
                    "score_source": str(score_src),
                    "shape_source": str(shape_src),
                    "shape_crs": "EPSG:5186",
                    "focal_emotions": ", ".join(FOCAL_EMOTIONS),
                    "minimum_count_rule": "non-null delta value in source workbook",
                }
            ),
            "region_emotion_shift": result,
        },
    )
    return FigureSource("Figure 7", out, [str(score_src), str(shape_src)], ["metadata", "region_emotion_shift"], {"region_emotion_shift": len(result)}, "One file includes manuscript-aligned focal emotion deltas for both flood events; geometry is loaded from the province boundary shapefile listed in metadata.")
